...
FunctionPass* llvm::createCpu0SEISelDAG(Cpu0TargetMachine &TM,CodeGenOpt::Level OptLevel)
{
    return new Cpu0SEDAGToDAGISel(TM,OptLevel);
}


std::pair<SDNode *,SDNode *> Cpu0SEDAGToDAGISel::selectMULT(SDNode *N,unsigned Opc,const SDLoc &DL,EVT Ty,bool HasLo, bool HasHi)
{
    SDNode *Lo =0,*Hi=0;
    SDNode *Mul = CurDAG->getMachineNode(Opc,DL,MVT::Glue,N->getOperand(0),N->getOperand(1));
...
void Cpu0SEDAGToDAGISel::selectAddESubE(unsigned MOp,SDValue InFlag,SDValue CmpLHS,const SDLoc &DL,SDNode *Node) const
{
