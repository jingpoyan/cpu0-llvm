...
#define DBEUG_TYPE "cpu0-subtarget"
...
static cl::opt<bool> EnableOverflowOpt("cpu0-enable-overflow",cl::Hidden,cl::init(false),cl::desc("Use trigger overflow instructions add and sub \ instead of non-overflow instructions addu and subu"));
...
Cpu0Subtarget::Cpu0Subtarget(const Triple &TT, const std::string &CPU,
  ...
  IsLittle(little), TM(_TM), TargetTriple(TT), TSInfo(),
      InstrInfo(
          Cpu0InstrInfo::create(initializezSubtargetDependencies(CPU, FS, TM))),
...
Cpu0Subtarget &Cpu0Subtarget::initializezSubtargetDependencies(StringRef CPU,StringRef FS,const TargetMachine &TM)
{
...
const Cpu0ABIInfo &Cpu0Subtarget::getABI() const {
    return TM.getABI();
}